...
GLuint projector_VAO;
std::vector<float> projector;
std::vector<unsigned int> projector_indices;
...
void init() {
    ...
    vec = axe.cone(1000, 0, 1, 1);
    n = 0;
    for(int i=0; i<vec.size(); ++i){
        for(int j=0; j<vec[i].size()/8; ++j){
            for(int k=0; k<8; ++k){
                projector.push_back(vec[i][8*j+k]);
            }
            if(j != 0){
                projector_indices.push_back(n-1);
                projector_indices.push_back(n);
            }
            ++n;
        }
        projector_indices.push_back(n-vec[i].size()/8);
        projector_indices.push_back(n-1);
    }
    initVAO(&projector_VAO, sizeof(GLfloat), projector.data(), projector.size(), projector_indices.data(), projector_indices.size(), {3, 3, 2}, 8, {0, 3, 6});
...
void setDirectedLight(Shader &shader, glm::vec3 direction, glm::vec3 color, float intensity){
    shader.setVec3("directed_light.direction", direction);
    shader.setVec3("directed_light.ambient",  0.2f, 0.2f, 0.2f);
    shader.setVec3("directed_light.diffuse",  intensity*color);
    shader.setVec3("directed_light.specular", intensity*glm::vec3(1.0f, 1.0f, 1.0f));
}

void setPointLightings(Shader &shader, glm::vec3 *lightPos, glm::vec3 *color, float* intensity, int n){
    for(int i=0; i<n; ++i){
        ...
        shader.setFloat(pref + "constant", 1.0f);
        shader.setFloat(pref + "linear", 0.14f);
        shader.setFloat(pref + "quadratic", 0.07f);
    }
}

void setProjectorLight(Shader &shader, glm::vec3 position, glm::vec3 direction, glm::vec3 color, float intensity, int cut_angle, int outer_cut_angle){
    shader.setVec3("projector_light.position", position);
    shader.setVec3("projector_light.direction", direction);
    shader.setVec3("projector_light.ambient",  0.2f, 0.2f, 0.2f);
    shader.setVec3("projector_light.diffuse",  intensity*color);
    shader.setVec3("projector_light.specular", intensity*glm::vec3(1.0f, 1.0f, 1.0f));
    shader.setFloat("projector_light.cutOff", cos(glm::radians((float)cut_angle)));
    shader.setFloat("projector_light.outerCutOff", cos(glm::radians((float)outer_cut_angle)));
}

bool inv_draw = false;

glm::vec3 directedLightDirection(-1.0f, 0.0f, 0.0f);
glm::vec3 directedLightColor(1.0f);
float directedLightIntensity = 1.0f;

glm::vec3 projectorLightPosition(-1.0f, 1.0f, 0.0f);
glm::vec3 projectorLightDirection(-1.0f, 0.0f, 0.0f);
glm::vec3 projectorLightColor(1.0f);
float projectorLightIntensity = 1.0f;
int cut_angle = 45;
int outer_cut_angle = 45;
...
void draw(Shader &shader, int draw_type, GLfloat side_ratio, glm::mat4 rotation, glm::mat4 model, GLuint VAO, int n, glm::vec3 diffuse) 
{
    ...
    setDirectedLight(shader, directedLightDirection, directedLightColor, directedLightIntensity);
    setPointLightings(shader, lightPos, color, intensity, 3);
    setProjectorLight(shader, projectorLightPosition, projectorLightDirection, projectorLightColor, projectorLightIntensity, cut_angle, outer_cut_angle);
...
void drawProjectorLighting(Shader &shader, float side_ratio, float scale, glm::vec3 lightPos, glm::vec3 lightDir, glm::vec3 color, float intensity, float cut_angle, GLuint lightingVAO, int n) {
    glm::mat4 model(1.0);
    glm::mat4 rotation(1.0);
    float phi, theta;
    phi = atan(lightDir.y/lightDir.x);
    if(lightDir.x < 0)
	phi += glm::radians(180.0f);
    theta = atan(lightDir.z/pow(lightDir.y*lightDir.y+lightDir.x*lightDir.x, 0.5));
    std::cout << phi << ' ' << theta << std::endl;
    model = glm::translate(model, lightPos);
    model = glm::rotate(model, theta, glm::vec3(sin(phi), -cos(phi), 0));
    model = glm::rotate(model, phi, glm::vec3(0.0f, 0.0f, 1.0f));
    model = glm::rotate(model, glm::radians(180.0f), glm::vec3(0.0f, 0.0f, 1.0f));
    glm::vec3 scaling(glm::radians(cut_angle));
    scaling = glm::tan(scaling);
    scaling.x = 1.0f;
    model = glm::scale(model, glm::vec3(scale)*scaling);
    model = glm::translate(model, glm::vec3(-1.0f, 0.0f, 0.0f));
    draw(shader, GL_LINES, side_ratio, rotation, model, lightingVAO, n, intensity * color);
}

void drawLighting(Shader &shader, float side_ratio, float scale, glm::vec3 lightPos, glm::vec3 color, float intensity, GLuint lightingVAO, int n) {
    glm::mat4 model(1.0);
    glm::mat4 rotation(1.0);
    float phi, theta;
    phi = atan(lightPos.x/lightPos.z);
    theta = atan(lightPos.y/pow(lightPos.z*lightPos.z+lightPos.x*lightPos.x, 0.5));
    model = glm::translate(model, lightPos);
    model = glm::rotate(model, theta, glm::vec3(cos(phi), 0.0f, -sin(phi)));
...
ImGui::StyleColorsDark();
	...
	for(int i=0; i<3; ++i)
	    drawLighting(lightingShader, width/height, 0.2f, lightPos[i], color[i], intensity[i], lightingVAO, 3*12);

	drawLighting(lightingShader, width/height, 5.0f, 
	     -50.0f*directedLightDirection/glm::length(directedLightDirection), 
	     directedLightColor, directedLightIntensity, lightingVAO, 3*12);

	drawProjectorLighting(lightingShader, width/height, 0.3f, 
	     projectorLightPosition, projectorLightDirection,
	     projectorLightColor, projectorLightIntensity, outer_cut_angle, 
	     projector_VAO, projector_indices.size());
    ...
    ImGui::Text("directed lighting");
    ImGui::SliderFloat3("direction", glm::value_ptr(directedLightDirection), -1.0, 1.0);
    ImGui::ColorEdit3("directed lighting color", glm::value_ptr(directedLightColor));
    ImGui::SliderFloat("directed lighting intensity", &directedLightIntensity, 0.0, 1.0);
    ...
    ImGui::Text("projector lighting");
    ImGui::SliderFloat3("projector position", glm::value_ptr(projectorLightPosition), -2.0, 2.0);
    ImGui::SliderFloat3("projector direction", glm::value_ptr(projectorLightDirection), -1.0, 1.0);
    ImGui::ColorEdit3("projector lighting color", glm::value_ptr(projectorLightColor));
    ImGui::SliderFloat("projector lighting intensity", &projectorLightIntensity, 0.0, 1.0);
    ImGui::SliderInt("outer cut off angle", &outer_cut_angle, 5, 60);
    ImGui::SliderInt("cut off angle", &cut_angle, 2, outer_cut_angle);
    if(cut_angle > outer_cut_angle)
	cut_angle = outer_cut_angle;
    ImGui::End();
